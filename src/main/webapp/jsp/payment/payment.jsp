...
request.setAttribute("seatLength", seatLength);
%>
<head>
<meta charset="utf-8" />
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/payment/globals.css" />
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/payment/style.css" />
</head>
<body>
	<div class="payment">
		<div class="overlap-wrapper">
			<div class="overlap">
				...
				<div class="total-container">
					<div class="infor-back">
						<div class="infor-text">예매정보</div>
					</div>
					<div class="pay-meth-back">
						<div class="pay-meth-text">결제수단</div>
					</div>
					<div class="pay-background">
						<div class="pay-back-text">결제하기</div>
					</div>
				</div>
								...
								<div class="frame-8">
									<div class="text-wrapper-24">
										...
										<c:if test="${fn:length(cvo)!=null and fn:length(cvo) > 0 }">
											<c:forEach var="item" items="${requestScope.cvo}"
												varStatus="vs">
												<div class="coupon" id="${vs.index}">
													${item.c_item.ci_content} <input type="hidden"
														id="form${vs.index}" value="${item.c_item.ci_discount}">
													<input type="hidden" id="couponform${vs.index}"
														value="${item.cp_no}">
												</div>
											</c:forEach>
	...
	<form action="" method="post">
		<!-- 원래 가격 -->
		<input type="hidden" id="OriginalPrice" name="hiddenOriginalPrice"
			value="<%=originalPrice%>">
		<!-- 할인 금액 -->
		<input type="hidden" id="Discount" name="hiddenDiscount"
			value="<%=saleAmount%>">
		<!-- 쿠폰 코드 -->
		<input type="hidden" id="coupon_id" name="hiddenDiscount" value="">
		
		<!-- 최종 금액 -->
		<input type="hidden" id="FinalPrice" name="hiddenFinalPrice"
			value="<%=finalPrice%>">
	</form>
...
$(function(){
  ...
  function pay_info(rsp){
	...
	/* 
	var cp_no = $('#coupon_id').val()
	input_info('cp_no', cp_no);
	 */
